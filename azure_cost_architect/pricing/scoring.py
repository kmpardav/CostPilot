...
def _item_text(it: Dict[str, Any]) -> str:
    ...
    return " ".join(
        [
            ...
            _low(_g(it, "productName", "product_name", "ProductName")),
...
def score_price_item(resource: Dict[str, Any], item: Dict[str, Any], hours_prod: int = 730) -> int:
    ...
    if category.startswith("storage.files"):
        ...
        t_files = _item_text(item)
        if any(k in t_files for k in ("files v2", "azure files", "file storage", "file share")):
            score += 40
        if any(k in t_files for k in ("data stored", "provisioned", "storage", "gb", "tb")):
            score += 25
        if any(k in t_files for k in ("operations", "transactions", "requests", "protocol operations")):
            # Avoid accidental selection of per-request meters when we want primary capacity.
            score -= 15
